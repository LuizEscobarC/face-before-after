...
2. Diretórios existem: `.claude/local/plans/`, `.claude/local/prompts/`.
...
- Salvar plano em `.claude/local/plans/[slug]-[YYYY-MM-DD].md`.
- Emitir seção obrigatória `## Recommended Execution Model` com `opus|sonnet|haiku`.

**Validação após Step 1:**
```bash
LATEST_PLAN=$(ls -t .claude/local/plans/*.md 2>/dev/null \
  | grep -v -E '(CLAUDE_CONTEXT|README)' \
  | head -1)
test -n "$LATEST_PLAN" || abort "❌ planner não gerou plano em .claude/local/plans/"
...
- Salvar em `.claude/local/prompts/[module]/task-[slug]-[date].prompt.md`.

**Validação após Step 2:**
```bash
LATEST_PROMPT=$(find .claude/local/prompts -name '*.prompt.md' -type f -printf '%T@ %p\n' 2>/dev/null \
...
- **Falha no Step 1:** abortar; reportar erro do planner. Nada foi gravado fora de `.claude/local/plans/`.
...
- `.claude/local/skills/planner/SKILL.md` — Step 1
- `.claude/local/skills/prompt-initializer/SKILL.md` — Step 2
- `.claude/local/skills/auto-execute-prompt/SKILL.md` — Step 3
- `.claude/local/plans/CLAUDE_CONTEXT.md` §8 — Auto Model Routing (Opção C+B)
