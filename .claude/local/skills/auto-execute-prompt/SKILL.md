...
1. Existe um plano em `.claude/local/plans/*.md` (não-CLAUDE_CONTEXT) com a seção:
   ```
   ## Recommended Execution Model
   - **Model:** opus | sonnet | haiku
   - **Reason:** ...
   ```
2. Existe um `.prompt.md` em `.claude/local/prompts/**/*.prompt.md` (gerado por `prompt-initializer`).
...
Se vazio: abortar com `❌ Nenhum plano encontrado em .claude/local/plans/. Rode /planner primeiro.`
...
- `.claude/local/plans/CLAUDE_CONTEXT.md` §8 — Auto Model Routing
- `.claude/local/skills/planner/SKILL.md` — emite o plano com Recommended Execution Model
- `.claude/local/skills/execute-prompt/SKILL.md` — workflow de execução
