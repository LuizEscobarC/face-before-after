...
module: "plans/PLAN_METRICS"
file_path: ".claude/plans/PLAN_METRICS.md"
doc_type: "decision"
created_at: "2026-05-24"
updated_at: "2026-05-24"
version: "1.0.0"
summary_context: >
  > Plano mestre da expansão de métricas, ideais, overlays e diagnóstico. > Documento de referência permanente. Releia no início de cada sessão. > Última atualização: 2026-05-11 PR-67/68/69 — Grupo D M4.5 PDF: PdfBuilder ReportLab + Nest POST /v1/analysis/:id/pdf + botão frontend.
tags:
  - "planning"
  - "metrics"
rag_keywords:
...
| **PR-41 (M3.4)** | **Python `BeforeIdealComposer` vetorial — composição side-by-side (Opus required)**. Novo módulo `backend/app/vision/services/before_ideal_composer.py`: `compose_before_ideal(image, landmarks, offsets, *, show_actual_wireframe=True, show_guide_lines=True, gutter_px=8)`. Retorna RGBA double-width (`2×W + gutter_px`). **Left pane = byte-identical original photo** (no-warp promise per DEC-15 — testado via `np.array_equal`). **Right pane** = photo + actual wireframe (gray solid alpha 160) + ideal wireframe (cyan `#22d3ee` dashed 4/2 per DEC-26) + guide lines opcionais (vertical midline + horizontal intercanthal em cyan faint). Dataclass `IdealLandmarkOffset(landmark_index, dx_icu, dy_icu, metric_id="")` frozen+slots; magnitude cap ±0.3 ICU mirror dos calculadores PR-34. Wireframe = `LM_JAWLINE` + brows (L/R) + eyes closed (L/R) + outer mouth closed + nose bridge. `BeforeIdealComposeError(reason ∈ {invalid_landmark_shape, insufficient_landmarks, intercanthal_distance_degenerate, invalid_landmark_index})`. Novo endpoint `POST /vision/compose-before-ideal` em `backend/app/vision/routers/compose.py` (multipart: `image` + `landmarks_json` + `offsets_json` default `"[]"` + `show_actual_wireframe` form bool + `show_guide_lines` form bool); wired em `backend/app/vision/router.py`. **10 unit tests** novos em `backend/tests/unit/test_before_ideal_composer.py` (output dimensions/mode, left pane byte-identical, empty offsets renderiza cyan, single offset shifta só right pane, magnitude cap clipping, ICD degenerado raise, insufficient landmarks raise, invalid index raise, toggle actual wireframe, toggle guide lines). Suite total: **1110 ✓ + 48 skipped** (era 1100 + 48). Commit: `2140a7e`. Fontes: PLAN_M3_OVERLAYS §2 PR-41 + DEC-15/DEC-26, PLAN_METRICS §2 DEC-15, Naini 2011 Ch.4-6 (https://link.springer.com/book/10.1007/978-1-4419-7080-7), Powell & Humphreys 1984, Farkas 1994, MediaPipe Mesh-478 (https://github.com/google-ai-edge/mediapipe/blob/master/docs/solutions/face_mesh.md), Pillow ImageDraw (https://pillow.readthedocs.io/en/stable/reference/ImageDraw.html). | ✅ DONE |
| **PR-42 (M3.4)** | **Nest: proxy estateless + persist `rendered_asset.asset_type='before_ideal_composition'`**. `vision.dto.ts`: `OffsetDto` + `ComposeBeforeIdealRequestDto`. `vision.client.ts`: `composeBeforeIdeal()` via native `fetch + FormData + Blob` multipart. `vision.controller.ts`: `POST /v1/vision/compose-before-ideal` (PNG stream para frontend, stateless). `overlays.service.ts`: `composeBeforeIdealAndPersist()` — carrega `LandmarkPayloadEntity` do DB, baixa imageUrl, chama Python multipart, faz upload MinIO (`rendered/{id}/{uuid}_before_ideal.png`), persiste `RenderedAssetEntity(assetType='before_ideal_composition')`. `overlays.controller.ts`: `POST /v1/overlays/:reportId/compose-before-ideal` (HTTP 201, retorna `RenderedAssetDto`). Commit: `d95e218`. Fontes: Nest Fastify (https://docs.nestjs.com/techniques/http-module), FormData+Blob Node.js 18 (https://nodejs.org/api/globals.html#class-formdata), MinIO JS SDK (https://github.com/minio/minio-js), PLAN_M3_OVERLAYS §2 PR-42, DEC-15, DEC-26. | ✅ DONE |
| **PR-43 (M3.4)** | **Frontend: ViewMode `"before_ideal"` em `PremiumResultPage.tsx`**. `COMPOSE_ANCHOR` mapping (`midline_deviation→1, chin_height_ratio→152, brow_height_l→107, brow_height_r→336`) espelha `IMPROVEMENT_ANCHOR` de `OverlayLayer.tsx` (MediaPipe Mesh-478). `buildComposeOffsets()` filtra `metric_evaluations` por `COMPOSE_ANCHOR` + non-null `improvement_vector_x/y`. `fetchCompose()` (`useCallback`): `POST /v1/vision/compose-before-ideal` (JSON) → blob → `URL.createObjectURL`. Sidebar "Vetores ideais" (📏) aparece quando `hasBeforeIdeal` (run_id + landmarks + ≥1 metric com vetor). Toggles: "Linhas guia" (`showGuideLines`) + "Wireframe atual" (`showActualWireframe`) re-disparam `fetchCompose`. View body: spinner | img | empty state. Cleanup: `URL.revokeObjectURL` em unmount. `npx tsc --noEmit`: zero erros. Commit: `d861a4e`. Fontes: MediaPipe Mesh-478 (https://github.com/google-ai-edge/mediapipe/blob/master/docs/solutions/face_mesh.md), MDN createObjectURL (https://developer.mozilla.org/en-US/docs/Web/API/URL/createObjectURL), PLAN_M3_OVERLAYS §2 PR-43, DEC-26. | ✅ DONE |
...
| **PR-55 (M4.3)** | **DDL `recommendation_catalog` + 4 tabelas**. Migration `1746000200000-M43RecommendationCatalog`: `recommendation_catalog_version`, `recommendation_catalog` (slug TEXT PK, category enum 6 valores, priority/effort/risk, professional_type CHECK), `recommendation_trigger` (mapeia metric×severity×direction → recomendação, JSONB `additional_conditions`), `recommendation_link` (instância por sessão, soft FK a `analysis_report` partitioned). Seed v0.1 = só version container. | ✅ DONE |
| **PR-55b (M4.3)** | **Migration `M44RecommendationLadder` — escada de invasividade**. `1746000225000-M44LadderEnumValues` (não-transacional): adiciona `'exercise'` e `'aesthetic_procedure'` ao `recommendation_category_enum` + cria `evidence_level_enum` (`strong|moderate|anecdotal`). `1746000230000-M44RecommendationLadder`: novas colunas em `recommendation_catalog` — `invasiveness_level` SMALLINT NOT NULL (0..4), `evidence_level` enum NOT NULL DEFAULT 'moderate', `requires_anecdotal_disclaimer` BOOLEAN GENERATED, `clinical_pathway_required` BOOLEAN, `references_jsonb` JSONB, `disclaimer_template` TEXT. CHECK `chk_rec_anecdotal_disclaimer` força disclaimer quando evidence='anecdotal'. CHECK `chk_rec_professional_type` estendido com `oral_maxillofacial_surgeon`. `recommendation_trigger` ganha `min_invasiveness_level` + `clinical_pathway_required`. Plano: `.claude/plans/fa-a-mais-uma-revis-o-quirky-hopper.md`. Fonte: DEC-34/37/38 revisados em M4_NARRATIVE.md em 2026-05-09. | ✅ DONE |
| **PR-56 (M4.3)** | **Catálogo Opus — 427 recomendações + 8.352 triggers** (vs ~50 originalmente planejadas). Três migrations de seed: `1746000235000-M44SeedExerciseCatalog` (20 exercícios baseline cobrindo 9 regiões), `1746000240000-M44SeedFullCatalog100` (91 itens canônicos curados pelo produto: postura, mioterapia, dieta, skincare, styling, sono, foto, professional_referral), `1746000245000-M44SeedFullCatalog270` (270 itens da "long tail": 201-400 do `solucoes_para_rotina.md` + LT 1-70 ultra-avançado — mioterapia segmentar, ATM clínico, ortóptica, fisio cervical avançada, liberação craniana). Distribuição final: 24 photo (0) · 2 presentation_only (0) · 30 posture (1) · 142 lifestyle (1) · 160 exercise (2) · 49 styling (3) · 27 professional_referral (4b). Evidência: 350 strong/moderate · 84 anecdotal com disclaimer obrigatório. Tom dos `professional_referral` reescrito conforme DEC-35 revisada (informativo não-prescritivo: "alguns usuários optam por consultar..."). Catálogo `aesthetic_procedure` (4a — botox, fillers, fios PDO) deferido a PR-56b. | ✅ DONE |
| **PR-60 (M5 — Admin UX)** | **`SvgAnatomicalHand` — componente SVG anatômico totalmente parametrizável**. 4 arquivos novos em `frontend/src/components/SvgAnatomicalHand/`: `types.ts` (`FingerConfig`, `ThumbConfig`, `HandConfig`, `HandAction`, `HandPresetName`, `FingerPoints`, `HandGeometry`), `kinematics.ts` (forward kinematics: `computeFinger`, `computeThumb`, `computeHandGeometry`, `fingerOutlinePath`, `palmPath` — sem paths hardcoded, tudo calculado por ângulo), `presets.ts` (10 posturas nomeadas: `open`, `fist`, `point`, `two_finger_press`, `pinch`, `massage`, `palm_press`, `ok_sign`, `chin_fist`, `brow_press`), `index.tsx` (`SvgAnatomicalHand` standalone + `SvgAnatomicalHandGroup` embeddable). Props: `config?`, `preset?`, `flip?`, `rotation?`, `scale?`, `x?`, `y?`, `color?`, `fillOpacity?`, `animate?`, `transition?`, `showJointDots?`, `showContactHalo?`, `action?` (`idle|press|pull|massage`), `svgWidth?`, `svgHeight?`. Framer Motion micro-animações por action. Build TypeScript limpo. | ✅ DONE |
| **PR-61 (M5 — Admin UX)** | **`landmarkToFaceState` + `useLiveFaceState` — bridge MediaPipe → SvgFaceInstructor**. `frontend/src/biometric/landmarkToFaceState.ts`: converte 478 landmarks + pose angles → `Partial<FaceState>` (brow lift/rotate, eye openness AR, jaw/mouth open, mouthScaleX, lipCornerLDy/R, mouthShape, cheekScale, faceRotate/Tilt). Normalização por IOD (svgIod=26px). `frontend/src/biometric/useLiveFaceState.ts`: hook `useLiveFaceState()` → `{faceState, isRunning, isInitializing, start, stop, error}`. Gerencia ciclo de vida: Comlink wrap do worker → `getUserMedia` → canvas offscreen → `detectVideoLandmarks` @ ~24fps via `setTimeout` loop. Cleanup de rAF + stream tracks em `stop()`. Worker (`mediapipe.worker.ts`) ganhou método `detectVideoLandmarks(ImageData) → LandmarkPayload | null` (VIDEO mode, inclui landmarks + pose — diferente de `analyzeVideoFrame` que retorna só `FeedbackResult`). | ✅ DONE |
| **PR-62 (M5 — Admin UX)** | **`SvgFaceInstructor` liveState prop + 3 novas seções em `/admin/animations/preview` + hand pairings em `/admin/biometric/preview`**. `SvgFaceInstructor`: novo prop `liveState?: Partial<FaceState>` — quando presente, bypassa config e usa `{...NEUTRAL_FACE_STATE, ...liveState}` como peak com `transition:{duration:0.05, ease:'linear'}` para tracking real-time. `AdminAnimationsPreviewPage`: seção "Mãos Anatômicas — Posturas" (grid de 10 PreviewCards), seção "Hand Editor Interativo" (15 joint sliders + rotation + flip toggle + preset dropdown + live preview) e seção "Live Face — MediaPipe" (toggle câmera + `useLiveFaceState` + `SvgFaceInstructor liveState`). `AdminBiometricPreviewPage`: `inferHandPairing(recId)` deriva preset/rotation/action por padrão de keyword no id; cada card de exercício ganhou `<SvgAnatomicalHand>` (bilateral para massage). Build TS limpo. | ✅ DONE |
| **PR-63 (bugfix)** | **MediaPipe WASM offline — "ModuleFactory not set" corrigido**. `FilesetResolver.forVisionTasks('/wasm')` no worker (era CDN `cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.18/wasm`). WASM runtime (6 arquivos, 33 MB) copiado de `node_modules/@mediapipe/tasks-vision/wasm/` para `frontend/public/wasm/` — servido diretamente pelo nginx da frontend (`Content-Type: application/wasm`). `useLiveFaceState` default model URL atualizado para `/models/face_landmarker.task` (modelo local já existia em `public/models/`, mesma estratégia do `ClientPhotoProcessor.ts`). Zero dependências externas em runtime para MediaPipe. | ✅ DONE |
| **PR-64 (M5 — Admin UX)** | **`LandmarkRig` — SVG facial renderizado direto dos 478 landmarks MediaPipe** [2026-05-11]. Substitui `FaceRig` hardcoded (anchors fixos `HEAD_CX=50/EYE_CY=50/MOUTH_CY=82` que descartavam 99% da informação dos landmarks). 5 arquivos novos: `frontend/src/biometric/normalizeLandmarks.ts` (`normalizeToViewBox` via 4 âncoras estáveis 10/152/234/454, fillRatio=0.85, viewBox 100×130), `frontend/src/biometric/oneEuroFilter.ts` (Casiez 2012, mincutoff=1.5/beta=0.05/dcutoff=1.0, `OneEuroFilterArray(478,3)` singleton no rig), `frontend/src/biometric/faceMeshTopology.ts` (FACE_OVAL/LIPS_OUTER/INNER/LEFT+RIGHT_EYE/EYEBROW/NOSE_BRIDGE/BOTTOM/IRIS + helpers `polylinePath/regionCenter/regionRadius`), `scripts/extract_canonical_mesh.py` (download `canonical_face_model.obj` do MediaPipe, parse, normaliza, gera JSON) + `frontend/src/biometric/canonicalFaceMesh.json` (468 pts, 11.7 KB), `frontend/src/components/ExerciseStudioPreview/LandmarkRig.tsx` (~290 linhas, render `<motion.path d=…>` por região + `applyFaceStateDeformation` no modo scripted: jaw, lips, eye openness, brow lift/rotate, lip-corner asymmetry; **pseudo-3D matrix preservada** por DEC do usuário — skewX/scaleY/rotate aplicados no outer `<g>` SOBRE landmarks já posicionados). Edits: `RecordedKeyframe.landmarks?: number[][]` (retro-compat), `useExerciseRecorder` grava landmarks junto do delta, `useRecordedTimelineFrame` retorna `{delta, landmarks}`, `StudioStyle` ganha 5 cores opcionais (`eyeFill/irisColor/lipColor/mouthInner/meshColor`). DELETE `FaceRig.tsx`. **Surpresa:** OBJ canônico oficial tem **468 vértices**, não 478 — íris (468-477) é injetada pelo modelo de íris em runtime; tratado via fallback `irisCenter()` ao centro do olho em modo scripted. Build TS+Vite limpo (432 módulos, bundle 459.38 kB / 146.30 kB gzip). Plano: `landmark-driven-face-rig-2026-05-11.md`. Fontes: Casiez et al. 2012 1€ Filter (https://gery.casiez.net/1euro/), MediaPipe canonical_face_model.obj (https://github.com/google-ai-edge/mediapipe/blob/master/mediapipe/modules/face_geometry/data/canonical_face_model.obj), MediaPipe Mesh-478 (https://github.com/google-ai-edge/mediapipe/blob/master/docs/solutions/face_mesh.md). | ✅ DONE |
| **PR-67 (M4.5 PDF)** | **Python `PdfBuilder` (ReportLab)** [2026-05-11]. `backend/app/vision/services/pdf_builder.py` (433 linhas). Estrutura: capa (título + score global colorido + data) → findings (até 3, badges de severidade) → rendered assets (até 4 imagens, `urllib.request`) → recomendações (até 5, nota para `professional_referral`) → disclaimer. Design tokens CMYK-safe espelham MVP palette (`--accent #6366f1`, `--accent2 #22d3ee`, `--surface2 #1c1c2e`, `--muted #94a3b8`). TypedDicts: `FindingData`, `RecommendationData`, `PdfReportData`. Helpers: `_score_color` (≥7.5 cyan / ≥5.5 amber / <5.5 red), `_severity_badge_color` (5 termos pt-BR), `_fetch_image_bytes` (timeout=10s, silently fails). Router `POST /vision/generate-pdf` wired em `backend/app/vision/router.py`. **24 unit tests** em `backend/tests/unit/test_pdf_builder.py` (magic bytes, min/full payload, capping, score_color boundaries, severity badges, edge-case scores 0.0/10.0, date parsing). Suite total: **1134 ✓ + 48 skipped** Python. | ✅ DONE |
| **PR-68 (M4.5 PDF)** | **Nest `PdfService` + 2 endpoints** [2026-05-11]. `PdfService.generateForReport(reportId)`: (1) carrega `AnalysisReportEntity` + assets existentes via `RenderedAssetEntity` (excluindo `report_pdf`); (2) presigned URLs via `MinioStorageService.storageUrlToPresigned`; (3) monta payload com `NarrativeService`; (4) chama `VisionClient.generatePdf` → Python `/vision/generate-pdf`; (5) upload `rendered/pdf/{reportId}.pdf` via `MinioStorageService.uploadBuffer`; (6) persiste `RenderedAssetEntity(assetType='report_pdf', byteSize)` com idempotência (deleta existente). `PdfController`: `POST /v1/analysis/:id/pdf` → retorna `{pdf_url, report_id, asset_id}`. `RunPdfController`: `POST /v1/analysis/run/:runId/pdf` → stateless, recebe `{global_score, findings, recommendations, disclaimer}` no body, streama PDF diretamente (Fastify `reply.send`). Ambos registrados em `DiagnosisModule`. `VisionClient.generatePdf(payload)` em `vision.client.ts`. | ✅ DONE |
| **PR-69 (M4.5 PDF)** | **Frontend: botão "↓ Baixar relatório (PDF)"** [2026-05-11]. `PremiumResultPage.tsx`: estado `pdfLoading: boolean` + `pdfError: string|null`. Handler `onClick`: monta payload com `result.score`, `result.findings`, `result.recommendations`; `POST /v1/analysis/run/${result.run_id}/pdf`; `URL.createObjectURL(blob)`; `<a download>` trigger; `URL.revokeObjectURL`. UX: botão disabled + texto "Gerando PDF…" durante loading; mensagem de erro inline. Posicionado no header da página Premium. `npx tsc --noEmit` clean. | ✅ DONE |
...
**M4.3 completo (PR-55..56)** [2026-05-09]: Catálogo de recomendações com **escada de invasividade de 5 níveis** + **8 categorias** (`photo`, `presentation_only`, `posture`, `lifestyle`, `exercise` *(novo)*, `styling`, `aesthetic_procedure` *(novo)*, `professional_referral`). PR-55 base + PR-55b adiciona `evidence_level`, `invasiveness_level` (0..4), `clinical_pathway_required`, `disclaimer_template`, `references_jsonb`. PR-56 popula **427 recomendações + 8.352 triggers** (3 migrations de seed: 20 exercícios + 91 itens canônicos + 270 long-tail). Tom de `professional_referral` reescrito conforme DEC-35: informativo não-prescritivo. Pendente: PR-56b (10–15 entradas de `aesthetic_procedure` nível 4a). Fontes: PLAN_M4_NARRATIVE §3 (DEC-34/35/37/38), `.claude/plans/fa-a-mais-uma-revis-o-quirky-hopper.md`, `solucoes_para_rotina.md` (curadoria do produto), Felício C.M. (J Oral Rehabil 2010), Solow & Sandham (Eur J Orthod 2002), Alam et al. (JAMA Dermatol 2018), McKenzie, Travell & Simons.

**M2 (em andamento)**: PR-12 entregou scoring infra; PR-13 (`jaw`, 6 métricas), PR-14 (`nose`, 7 métricas), PR-15 (`mouth/lips`, 7 métricas), PR-16 (`brows`, 8 métricas), PR-17 (`cheekbones`, 5 métricas), PR-18 (`forehead`, 4 métricas — 3 activas + 1 pixel-dep stub), PR-19 (`global_shape`, 4 métricas — 2 activas + 1 presentation_only + 1 pixel-dep stub), PR-20 (`phi/golden`, 4 métricas — TODAS presentation_only=True, DEC-6, sem ideais, sem pesos), PR-23 (multi-foto / landmark stability — infra de confiança), PR-21 v1.5 PROVISIONAL (rebalance estrutural — 9 regiões em global_weight), PR-24 (GET report retrieval) e PR-25 (Catalog API) já entregues. Total: **150 testes Vitest, 1088 testes Python**. DB com 66 metric_definitions, 59 metric_ideals. Próximas fatias do M2: **PR-22 (calibração com fotos reais — tarefa humana, ≥30 fotos)** → habilita promoção PR-21 v1.5→v2.0 (ver [`PLAN_PR21_V15_TO_V20_PROMOTION.md`](./marcos/M2_PR21_v15_to_v20.md) e [`PLAN_PR22_REAL_PHOTOS.md`](./marcos/M2_PR22_real_photos.md)). Detalhe completo em [`PLAN_M2_BACKLOG.md`](./marcos/M2_BACKLOG.md).
...
- `IdealComparator` (Nest) consome `metric_value` (vindo do Python) + `metric_ideal` → produz `deviation_raw`, `deviation_normalized`, `direction_label`
  - `deviation_normalized = deviation_raw / sided_half_width` onde `sided_half_width = (greenMax − central)` quando `deviation_raw ≥ 0`, senão `(central − greenMin)`. **Half-width "do lado certo"** — corrige (a) métricas unilaterais (assimetrias com `greenMin == central`) que antes colapsavam half-width em 0 e geravam `dn=null`, descartadas silenciosamente do score; e (b) ranges verdes assimétricas (ex: `[0.85, 1.20]` em centro 1.00) que antes produziam falsos `mild` ao penalizar o lado largo com a half-width estreita.
- `SeverityClassifier` (Nest) usa `severity_collapse_policy` para mapear 5→3
- Score por região: `quality_i = clip(1 - |deviation_normalized| / 5.0, 0, 1)`; soma `quality_i · weight_i · confidence_final_i` ponderada por `region_metric_weights.yaml`. **Saturação dn=5.0** alinha com `SeverityClassifier.STRONG_MAX` para que a curva de qualidade degrade linearmente de `ideal` (q=1) a `extreme` (q=0), preservando a gradação entre `mild/moderate/strong/extreme` (antes a fórmula `1 - |dn|` saturava em `|dn|=1`, achatando toda severidade não-ideal em q=0).
...
- **M2** (🟡 em andamento, primeira fatia entregue no PR-12): famílias restantes (mandíbula, nariz, boca, sobrancelhas, maçãs, testa, global) → 60+ métricas. Score regional + global com banding (✅ PR-12). Calibração com 30-50 fotos reais. **Backlog detalhado em [`PLAN_M2_BACKLOG.md`](./marcos/M2_BACKLOG.md)**.
- **M3**: overlays SVG no frontend + heatmaps + composição before/ideal vetorial + endpoint `POST /vision/render`. **Plano detalhado em [`PLAN_M3_OVERLAYS.md`](./marcos/M3_OVERLAYS.md)**.
- **M4**: templates de texto + priorização + recomendações catalogadas + PDF do relatório + disclaimers. **Plano detalhado em [`PLAN_M4_NARRATIVE.md`](./marcos/M4_NARRATIVE.md)**.
